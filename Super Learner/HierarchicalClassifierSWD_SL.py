...
class SuperLearnerClassifier(object):
	def __init__(self, node, first_level_learners):
		...
		for first_level_learner in first_level_learners:
			if first_level_learner.startswith('w'):
				self.first_level_classifiers.append(SklearnWekaWrapper(self.classifier_name))
			elif first_level_learner.startswith('d'):
				self.first_level_classifiers.append(SklearnSparkWrapper(self.classifier_name))

	def fit(self, dataset, labels):
		
		training_set, ground_truth, _, _ = train_test_split(dataset, labels, train_size=0.2)
	...
	def _first_level_fit(self, dataset, labels):
		...
		for train_index, test_index in skf.split(dataset, labels):
			...
			prediction = np.ndarray(shape=[len(test_index), first_level_classifiers_number],dtype=int)
			probability = np.ndarray(shape=[len(test_index), first_level_classifiers_number],dtype=object)

			for i, first_level_classifier in enumerate(first_level_classifiers):
	...
	def _first_level_final_fit(self, dataset, labels):
		...
		for first_level_classifier in first_level_classifiers:
	...
	def _first_level_predict(self, testing_set):
		...
		for first_level_classifier in first_level_classifiers:
			...
			pred = first_level_classifier.predict(dataset, labels)
	...
	def _meta_learner_predict(self, testing_set):
		...
		predictions = self._predict_Decision_Template(np.transpose(testing_set), self.weights)
...
class HierarchicalClassifier(object):
	...
	def recursive(self,parent):
		...
		for i in range(parent.children_number):
			...
			child.encoder = LabelEncoder()
			child.encoder.fit(self.ground_truth[child.train_index, child.level])
	...
	def train(self, node, classifier):
		...
		node.test_duration = classifier.fit(
			node.encoder.transform(self.training_set[node.train_index][:, node.features_index]),
	...
	def test(self, node, classifier):
		...
		pred = node.encoder.inverse_transform(
			classifier.predict(
			node.encoder.transform(self.testing_set[node.test_index][:, node.features_index])
			))
	...
	def test_all(self, node, classifier):
		...
		pred = node.encoder.inverse_transform(
			classifier.predict(
			node.encoder.transform(self.testing_set[node.test_index_all][:, node.features_index])
			))
